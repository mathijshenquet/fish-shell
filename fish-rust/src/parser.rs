...
use crate::common::{
    ...
    EscapeStringStyle, FilenameRef, ScopeGuard, ScopeGuarding, PROFILING_ACTIVE,
};
use crate::complete::CompletionList;
use crate::env::status::ENV_OK;
use crate::env::{
    EnvDynFFI, EnvMode, EnvStack, EnvStackRef, EnvStackRefFFI, EnvStackSetResult, Environment,
    Statuses,
};
...
use cxx::{CxxString, CxxVector, CxxWString, UniquePtr};
...
impl Parser {
    ...
    pub fn eval_node<T: Node>(
        ...
        let zelf = scoped_push_replacer(
            self,
            |zelf, new_value| ParseExecutionContext::swap(&zelf.execution_context, new_value),
        ...
        job_reap(zelf, false); // reap again
    ...
    /// todo!("later: this RAII object should only be used for iterating over it (in reverse). Maybe enforce this")
