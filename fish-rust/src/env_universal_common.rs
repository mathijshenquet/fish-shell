use crate::common::{
    cstr2wcstring, read_loop, str2wcstring, timef, unescape_string, valid_var_name, wcs2zstring,
    write_loop, UnescapeFlags, UnescapeStringStyle,
};
...
impl EnvUniversal {
    ...
    pub fn initialize_at_path(&mut self, callbacks: &mut CallbackDataList, path: WString) {
        ...
        if self.load_from_path(&default_vars_path(), callbacks) {
    ...
    fn load_from_path(&mut self, path: &wstr, callbacks: &mut CallbackDataList) -> bool {
        self.narrow_vars_path = wcs2zstring(path);
        self.load_from_path_narrow(callbacks)
    }
    ...
    fn save(&mut self, directory: &wstr) -> bool {
        ...
        if success {
            ...
            if let Ok(md) = wstat(&real_path) {
                ...
                if unsafe { libc::fchmod(private_fd.fd(), md.mode()) } == -1 {
...
fn r#match(inout_cursor: &mut &wstr, cmd: &[u8]) -> bool {
    ...
    if !cursor
        .chars()
        .zip(cmd)
        .all(|(c1, c2)| c1 == char::from(*c2))
    {
        return false;
    }
    let len = cmd.len();
    if !matches!(cursor.char_at(len), ' ' | '\t') {
        return false;
    }
