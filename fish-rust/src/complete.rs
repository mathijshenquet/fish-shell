...
use crate::{
    ...
    common::{
        escape_string, str2wcstring, unescape_string, valid_var_name_char, EscapeStringStyle,
        ScopeGuard, UnescapeFlags, UnescapeStringStyle,
    },
    ...
    path::{path_get_path, path_try_get_path, GetPathResult},
...
impl CompletionReceiver {
    ...
    #[must_use]
    pub fn add_string(&mut self, completion: WString) -> bool {
        self.add(Completion::new(
            completion,
            WString::new(),
            StringFuzzyMatch::exact_match(),
            CompleteFlags::empty(),
    ...
    /// Returns `true` on success, `false` if this would overflow the limit.
    #[must_use]
    pub fn add_list(&mut self, lst: Vec<Completion>) -> bool {
        let Some(total_size) = self.size().checked_add(lst.len()) else {
            return false;
        };
        ...
        if self.completions.is_empty() {
            self.completions = lst;
        } else {
            self.completions.extend(lst);
        }
    ...
    pub fn subreceiver(&self) -> CompletionReceiver {
        CompletionReceiver::new(self.limit.saturating_sub(self.size()))
    }
}
...
pub fn sort_and_prioritize(comps: &mut Vec<Completion>, flags: CompletionRequestOptions) {
    ...
    if flags.autosuggestion {
        comps.sort_by(|a, b| match (a.r#match.case_fold, b.r#match.case_fold) {
            (CaseFold::samecase, CaseFold::samecase)
            | (CaseFold::smartcase, CaseFold::smartcase)
            | (CaseFold::icase, CaseFold::icase) => {
                compare_completions_by_duplicate_arguments(a, b)
                    .then_with(|| compare_completions_by_tilde(a, b))
            }
            (CaseFold::samecase, CaseFold::smartcase)
            | (CaseFold::samecase, CaseFold::icase)
            | (CaseFold::smartcase, CaseFold::icase) => Ordering::Less,
            (CaseFold::smartcase, CaseFold::samecase)
            | (CaseFold::icase, CaseFold::samecase)
            | (CaseFold::icase, CaseFold::smartcase) => Ordering::Greater,
        })
    }
}
...
impl<'ctx> Completer<'ctx> {
    ...
    pub fn perform_for_commandline(&mut self, cmdline: WString) {
        ...
        let _decrement = if let Some(parser) = self.ctx.maybe_parser() {
            ...
            *level += 1;
            ...
            Some(ScopeGuard::new(parser, |parser| {
                let level = &mut parser.libdata_mut().pods.complete_recursion_level;
                *level -= 1;
            }))
        } else {
    ...
    fn complete_cmd_desc(&mut self, s: &wstr) {
        ...
        let lookup_cmd: WString = [
            L!("__fish_describe_command "),
            &escape_string(cmd, EscapeStringStyle::default()),
        ]
        .into_iter()
        .collect();
    ...
    fn complete_from_args(&mut self, s: &wstr, args: &wstr, desc: &wstr, flags: CompleteFlags) {
        ...
        self.complete_strings(
            &escape_string(s, EscapeStringStyle::default()),
            &const_desc(desc),
            &possible_comp,
            flags,
    ...
    fn complete_param_expand(&mut self, s: &wstr, do_file: bool, handle_as_special_cd: bool) {
        ...
        if let Some(sep_index) = sep_index {
            ...
            if !self.completions.add_list(local_completions) {
                return;
            }
        }
    ...
    fn complete_variable(&mut self, s: &wstr, start_offset: usize) -> bool {
        ...
        for env_name in self.ctx.vars().get_names(EnvMode::empty()) {
            ...
            let (comp, flags) = if !r#match.requires_full_replacement() {
                ...
                let comp = whole_var.slice_from(start_offset).to_owned() + env_name.as_utfstr();
    ...
    fn try_complete_user(&mut self, s: &wstr) -> bool {
        ...
        // FIXME: if getpwent() unavailable (e.g. android), return false
        ...
        fn getpwent_name() -> Option<WString> {
            ...
            Some(str2wcstring(pw_name.to_bytes()))
        }
    ...
    fn apply_var_assignments<'a>(
        ...
        for var_assign in var_assignments {
            let Some(equals_pos) = variable_assignment_equals_pos(var_assign) else {
                panic!("All variable assignments should have equals position");
            };
...
fn param_match2<'s>(e: &CompleteEntryOpt, optstr: &'s wstr) -> Option<&'s wstr> {
    // FLOG!(error, "MATCHING", optstr, "AGAINST OPT", &e.option);
...
fn append_switch_short_arg(out: &mut WString, opt: char, arg: &wstr) {
    ...
    sprintf!(=> out, " -%lc %ls", opt, escape_string(arg, EscapeStringStyle::default()));
}
fn append_switch_long_arg(out: &mut WString, opt: &wstr, arg: &wstr) {
    ...
    sprintf!(=> out, " --%ls %ls", opt, escape_string(arg, EscapeStringStyle::default()));
}
...
fn completion2string(index: &CompletionEntryIndex, o: &CompleteEntryOpt) -> WString {
    ...
    if index.is_path {
        ...
        out.push(' ');
        out.push_utfstr(&escape_string(&index.name, EscapeStringStyle::default()));
    }
...
pub fn complete_print(cmd: &wstr) -> WString {
    ...
    for (src, targets) in wrappers.iter() {
        ...
        for target in targets {
            ...
            out.push_utfstr(&escape_string(src, EscapeStringStyle::default()));
