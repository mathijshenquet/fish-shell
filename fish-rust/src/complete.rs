...
impl CompletionReceiver {
    ...
    /// \return true on success, false if this would overflow the limit.
    #[must_use]
    pub fn extend(&mut self, iter: impl IntoIterator<Item = Completion>) -> bool {
        let vec: Vec<Completion> = iter.into_iter().collect();
        if vec.len() > self.limit - self.completions.len() {
            return false;
        }
        self.completions.extend(vec);
        true
    }
    ...
    #[must_use]
    pub fn get_mut(&mut self, idx: usize) -> Option<&mut Completion> {
        self.completions.get_mut(idx)
    }
