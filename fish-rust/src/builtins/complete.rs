...
pub fn complete(
    ...
    // todo!("later: these whould be Vec<&wstr>");
    let mut gnu_opt = vec![];
    let mut old_opt = vec![];
    ...
    while let Some(opt) = w.wgetopt_long() {
        match opt {
            ...
            'C' => {
                do_complete = true;
                do_complete_param = do_complete_param.or(w.woptarg().map(|s| s.to_owned()));
            }
            OPT_ESCAPE => {
    ...
    if w.woptind != argc {
        ...
        if do_complete && do_complete_param.is_none() && argc == w.woptind + 1 {
            do_complete_param = Some(w.argv()[argc - 1].to_owned());
        } else if !do_complete && cmd_to_complete.is_empty() && argc == w.woptind + 1 {
            // Or use one left-over arg as the command to complete
            cmd_to_complete.push(w.argv()[argc - 1].to_owned());
        } else {
            streams
                .err
                .append(&wgettext_fmt!(BUILTIN_ERR_TOO_MANY_ARGUMENTS, w.cmd()));
    ...
    for condition_string in &condition {
        ...
        if parse_util_detect_errors(&condition_string, Some(&mut errors), false).is_err() {
            for error in errors {
                let prefix = w.cmd().to_owned() + L!(": -n '") + &condition_string[..] + L!("': ");
    ...
    if !comp.is_empty() {
        ...
        prefix.push_utfstr(w.cmd());
        prefix.push_str(": ");

        if let Err(err_text) = parse_util_detect_errors_in_argument_list(&comp, &prefix) {
            streams
                .err
                .append(&wgettext_fmt!("%ls: %ls: contains a syntax error\n"));
        ...
        let do_complete_param = match do_complete_param {
            None => {
                ...
                if !ffi::commandline_get_state_initialized_ffi() {
                    ...
                    // See #2361    .
                    builtin_missing_argument(parser, streams, w.cmd(), L!("-C"), true);
                    return STATUS_INVALID_ARGS;
                }
