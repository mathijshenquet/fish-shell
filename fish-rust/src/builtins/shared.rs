...
use crate::ffi;
use crate::ffi::Repin;
use crate::io::{DynOutputStreamFfi, IoChain, IoFd, OutputStream, SeparationType};
...
use cxx::{type_id, CxxWString, ExternType};
use libc::isatty;
...
pub fn builtin_run(
    ...
    let Some(builtin) = builtin_lookup(&argv[0]) else {
        FLOGF!(
            error,
            "%s",
            wgettext_fmt!(UNKNOWN_BUILTIN_ERR_MSG, &argv[0])
        );
...
mod builtins_ffi {
    extern "C++" {
        ...
        type DynOutputStreamFfi<'a> = crate::io::DynOutputStreamFfi<'a>;
    ...
    extern "Rust" {
        ...
        unsafe fn builtin_print_error_trailer_ffi<'a>(
            parser: &Parser,
            b: Pin<&mut DynOutputStreamFfi<'a>>,
            cmd: &CxxWString,
        );
    }
}
...
fn builtin_unknown_option_ffi<'a>(
    parser: &Parser,
    streams: Pin<&mut IoStreams<'a>>,
    cmd: &CxxWString,
    opt: &CxxWString,
