        ...
        while let Some(c) = w.wgetopt_long() {
            match c {
                ...
                '?' => {
                    ...
                    builtin_unknown_option(parser, streams, w.cmd(), &w.argv()[optind - 1], false);
                    return Err(STATUS_INVALID_ARGS);
                }
                _ => {
...
fn list(opts: &Options, parser: &Parser, streams: &mut IoStreams<'_>) -> Option<c_int> {
    ...
    for key in names {
        ...
        if !names_only {
            ...
            } else if let Some(var) = parser.vars().getf_unless_empty(&key, compute_scope(opts)) {
