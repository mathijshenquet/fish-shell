...
use super::prelude::*;
use crate::io::{IoBuffer, IoBufferfill};
use crate::parser::BlockType;
use crate::wcstringutil::join_strings;
use libc::{STDERR_FILENO, STDOUT_FILENO};

pub fn eval(parser: &Parser, streams: &mut IoStreams<'_>, args: &mut [WString]) -> Option<c_int> {
    let argc = args.len();
    if argc <= 1 {
    ...
    if res.was_empty {
        ...
        status = Some(res.status.status_value());
    }
    ...
    if let Some(stderr) = stderr_fill {
        ...
        streams.out.append_narrow_buffer(&errput);
    }
