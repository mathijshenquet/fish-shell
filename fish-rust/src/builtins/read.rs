        ...
        match opt {
            ...
            'n' => {
                opts.nchars = match fish_wcstoi(w.woptarg().unwrap()) {
                    Ok(n) => n as usize, // historical behavior
                    Err(err) => {
        ...
        if bytes_consumed < bytes_read {
            ...
            if do_seek
                && unsafe {
                    libc::lseek(
                        fd,
                        (bytes_consumed as isize - (bytes_read as isize) + 1) as libc::off_t,
                        SEEK_CUR,
                    )
                } == -1
            {
...
fn validate_read_args(
    ...
    if let Some(ref mut prompt_str) = &mut opts.prompt_str {
        *prompt_str = L!("echo ").to_owned() + &escape(prompt_str)[..];
        opts.prompt = Some(prompt_str.clone());
    } else if opts.prompt.is_none() {
        opts.prompt = Some(DEFAULT_READ_PROMPT.to_owned());
    }

    if opts.place.contains(EnvMode::UNEXPORT) && opts.place.contains(EnvMode::UNEXPORT) {
        streams
            .err
...
pub fn read(parser: &Parser, streams: &mut IoStreams<'_>, argv: &mut [WString]) -> Option<c_int> {
