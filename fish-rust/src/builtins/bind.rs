//! Implementation of the bind builtin.
use std::{ffi::CString, sync::MutexGuard};

use crate::null_terminated_array::{AsNullTerminatedArray, OwningNullTerminatedArray};

use super::prelude::*;

const BIND_INSERT: c_int = 0;
const BIND_ERASE: c_int = 0;
const BIND_KEY_NAMES: c_int = 0;
const BIND_FUNCTION_NAMES: c_int = 0;

struct Options {
    all: bool,
    bind_mode_given: bool,
    list_modes: bool,
    print_help: bool,
    silent: bool,
    use_terminfo: bool,
    have_user: bool,
    user: bool,
    have_preset: bool,
    preset: bool,
    mode: c_int,
    bind_mode: &'static wstr,
    sets_bind_mode: &'static wstr,
}

pub fn bind(parser: &Parser, streams: &mut IoStreams<'_>, args: &mut [WString]) -> Option<c_int> {
    run_builtin_ffi(crate::ffi::builtin_bind, parser, streams, args)
}
