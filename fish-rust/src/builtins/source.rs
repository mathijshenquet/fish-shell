use crate::{
    common::{escape, scoped_push, scoped_push_replacer, FilenameRef},
...
use libc::{c_int, c_void, O_RDONLY};
...
/// context.
pub fn source(parser: &Parser, streams: &mut IoStreams<'_>, args: &mut [WString]) -> Option<c_int> {
    let argc = args.len();
    ...
    if argc == optind || args[optind] == L!("-") {
        ...
        func_filename = FilenameRef::new(args[optind].clone());
    }
