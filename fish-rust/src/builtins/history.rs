...
use crate::ffi::{self};
use crate::history::{self, history_session_id, History, HistorySearch};
...
impl TryFrom<&WString> for HistCmd {
    type Error = ();
    fn try_from(val: &WString) -> Result<Self, ()> {
        match val {
...
const longopts: &[woption] = &[
    ...
    wopt(L!("show-time"), woption_argument_t::no_argument, 't'),
    wopt(L!("exact"), woption_argument_t::no_argument, 'e'),
    wopt(L!("max"), woption_argument_t::no_argument, 'n'),
    ...
    wopt(L!("clear-session"), woption_argument_t::no_argument, '\x06'),
...
fn parse_cmd_opts(
    ...
    while let Some(opt) = w.wgetopt_long() {
        match opt {
            '\x01' => {
                if !set_hist_cmd(w.cmd(), &mut opts.hist_cmd, HistCmd::HIST_DELETE, streams) {
                    return STATUS_CMD_ERROR;
                }
            }
            '\x02' => {
                if !set_hist_cmd(w.cmd(), &mut opts.hist_cmd, HistCmd::HIST_SEARCH, streams) {
                    return STATUS_CMD_ERROR;
                }
            }
            '\x03' => {
                if !set_hist_cmd(w.cmd(), &mut opts.hist_cmd, HistCmd::HIST_SAVE, streams) {
                    return STATUS_CMD_ERROR;
                }
            }
            '\x04' => {
                if !set_hist_cmd(w.cmd(), &mut opts.hist_cmd, HistCmd::HIST_CLEAR, streams) {
                    return STATUS_CMD_ERROR;
                }
            }
            '\x05' => {
                if !set_hist_cmd(w.cmd(), &mut opts.hist_cmd, HistCmd::HIST_MERGE, streams) {
                    return STATUS_CMD_ERROR;
                }
            }
            'C' => {
            ...
            'n' => match fish_wcstol(w.woptarg().unwrap()) {
                Ok(x) => opts.max_items = Some(x as _), // todo!("later: historical behavior is to cast")
                Err(_) => {
                    streams.err.append(&wgettext_fmt!(
    ...
    match opts.hist_cmd {
        ...
        HistCmd::HIST_DELETE => {
            ...
            for delete_string in args {
                history.remove(delete_string.clone());
            }
        }
        HistCmd::HIST_CLEAR => {
