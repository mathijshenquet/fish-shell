...
pub fn fg(parser: &Parser, streams: &mut IoStreams<'_>, argv: &mut [WString]) -> Option<c_int> {
    ...
    let job;
    let mut job_pos = 0; // todo!("check if this is in sync")
    ...
    if optind == argv.len() {
        ...
        match fish_wcstoi(&argv[optind]) {
            ...
            Ok(pid) => {
                let pid = pid.abs();
                let j = parser.job_get_from_pid_at(pid);
                if j.as_ref()
    ...
    let _ = make_fd_blocking(STDIN_FILENO);
    {
        ...
        if job_group.wants_terminal() && job_group.tmodes.is_some() {
            let termios = job_group.tmodes.as_ref().unwrap();
