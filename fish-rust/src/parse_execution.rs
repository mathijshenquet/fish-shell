...
use crate::ast::{
    self, Ast, BlockStatementHeaderVariant, Keyword, Leaf, List, Node, StatementVariant, Token,
};
use crate::builtins;
...
use crate::common::{
    escape, scoped_push, scoped_push_replacer, should_suppress_stderr_for_tests, valid_var_name,
    ScopeGuard, ScopeGuarding,
};
use crate::complete::{Completion, CompletionList};
...
use crate::flog::FLOG;
use crate::function;
use crate::function::exists;
use crate::io::{IoChain, IoStreams, NullOutputStream, OutputStream, StringOutputStream};
...
impl<'a> ParseExecutionContext {
    ...
    fn eval_statement(
        ...
        match &**contents {
            ...
            StatementVariant::NotStatement(_) => panic!(),
    ...
    fn infinite_recursive_statement_in_job_list<'b>(
        ...
        let Some(jc) = &jobs.get(0) else { return None };
        let job = &jc.job;
    ...
    fn populate_job_process(
        ...
        let ctx = ScopeGuard::new(ctx, |ctx| {
    ...
    fn expand_arguments_from_nodes(
        ...
        for arg_node in argument_nodes {
            ...
            for new_arg in &mut arg_expanded {
                out_arguments.push(std::mem::take(&mut new_arg.completion));
            }
        }
    ...
    fn run_1_job(
        ...
        if zelf.job_is_simple_block(job_node) {
            ...
            let mut block = None;
            let mut result =
                zelf.apply_variable_assignments(&ctx, None, &job_node.variables, &mut block);
            let ctx = ScopeGuard::new(ctx, |ctx| {
                if let Some(block) = block {
                    ctx.parser().pop_block(block);
                }
            ...
            if result == EndExecutionReason::ok {
                result = match &**specific_statement {
                    StatementVariant::BlockStatement(block_statement) => {
                        zelf.run_block_statement(&ctx, block_statement, associated_block)
                    }
                    StatementVariant::IfStatement(ifstmt) => {
                        zelf.run_if_statement(&ctx, ifstmt, associated_block)
                    }
                    StatementVariant::SwitchStatement(switchstmt) => {
                        zelf.run_switch_statement(&ctx, switchstmt)
                    }
        ...
        let pop_result =
            zelf.populate_job_from_job_node(&ctx, &mut job, job_node, associated_block);
        let ctx = ScopeGuarding::commit(ctx);

        // Clean up the job on failure or cancellation.
        if pop_result == EndExecutionReason::ok {
            zelf.setup_group(ctx, &mut job);
    ...
    fn setup_group(&self, ctx: &OperationContext<'_>, j: &mut Job) {
        ...
        j.group_mut()
            .is_foreground
            .store(!j.is_initially_background());
    ...
    let src_end = match node {
        StatementVariant::BlockStatement(block_statement) => {
            ...
            match &**block_header {
                ...
                BlockStatementHeaderVariant::WhileHeader(while_header) => while_header
                    .condition
                    .semi_nl
                    .as_ref()
                    .unwrap()
...
fn remove_job(parser: &Parser, job: &JobRef) -> bool {
    let num_jobs = parser.jobs().len();
    for i in 0..num_jobs {
        if Rc::ptr_eq(&parser.jobs()[i], job) {
