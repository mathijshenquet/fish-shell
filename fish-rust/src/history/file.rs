...
use crate::{
    common::{str2wcstring, wcs2string},
...
impl TryFrom<MmapRegion> for HistoryFileContents {
    ...
    fn try_from(region: MmapRegion) -> Result<Self, Self::Error> {
        // todo!("maybe add back support for the old format");
        let type_ = infer_file_type(&region).ok_or(())?;
        if type_ != HistoryFileType::Fish2_0 {
            return Err(());
        }
...
pub fn append_history_item_to_buffer(item: &HistoryItem, buffer: &mut String) {
    assert!(item.should_write_to_disk(), "Item should not be persisted");

    let cmd = wcs2string(item.str());
    let mut cmd = String::from_utf8(cmd).expect("Non-UTF-8 locale");
    escape_yaml_fish_2_0(&mut cmd);
    writeln!(buffer, "- cmd: {}", cmd).unwrap();
    ...
    if !paths.is_empty() {
        ...
        for path in paths {
            let path = wcs2string(path);
            let mut path = String::from_utf8(path).expect("Non-UTF-8 locale");
...
fn replace_all(s: &mut String, needle: &str, replacement: &str) {
    let mut offset = 0;
    while let Some(mut pos) = s[offset..].find(needle) {
        pos += offset;
...
fn escape_yaml_fish_2_0(s: &mut String) {
    replace_all(s, "\\", "\\\\"); // replace one backslash with two
    replace_all(s, "\n", "\\n"); // replace newline with backslash + literal n
}

/// This function is called frequently, so it ought to be fast.
fn unescape_yaml_fish_2_0(s: &mut String) {
    // SAFETY: we only ever add and remove ASCII characters, leaving multi-byte UTF-8 codepoints
    // as-is
    ...
    while cursor < s.len() {
        ...
        match escaped_char {
            ...
            b'n' => {
                ...
                s.splice(backslash..=(backslash + 1), [b'\n']);
            }
            _ => {
...
fn read_line(data: &[u8]) -> (usize, &str) {
    // Locate the newline.
    if let Some(newline) = data.iter().position(|&c| c == b'\n') {
        // we found a newline
        let line =
            std::str::from_utf8(&data[..newline]).expect("history file contains invalid UTF-8");
...
fn trim_leading_spaces(s: &str) -> (usize, &str) {
    if let Some(nonspace) = s.find(|c| c != ' ') {
        (nonspace, &s[nonspace..])
    } else {
        (s.len(), "")
    }
...
fn extract_prefix_and_unescape_yaml(line: &str) -> Option<(String, String)> {
...
fn decode_item_fish_2_0(mut data: &[u8]) -> Option<HistoryItem> {
    ...
    data = &data[advance..];
    let cmd = WString::from(value.as_str());
    ...
    loop {
        ...
        if key == "when" {
            ...
            when = Some(time_from_seconds(value.parse().unwrap_or(0)));
        } else if key == "paths" {
            ...
            loop {
                ...
                paths.push(WString::from(line.as_str()));
            }
        }
    }
...
fn time_to_seconds(ts: SystemTime) -> i64 {
...
fn parse_timestamp(s: &str) -> Option<SystemTime> {
    ...
    let Some(s) = s.strip_prefix("when:") else {
        return None;
    };

    let s = s.trim_start();

    s.parse().map(time_from_seconds).ok()
}
    ...
    while let Some(mut line) = lines.next() {
        ...
        if let Some(cutoff_timestamp) = cutoff_timestamp {
            ...
            loop {
                ...
                let Ok(interior_line) = std::str::from_utf8(interior_line) else {
                    // Invalid UTF-8, skip
                    continue;
                };
