...
use crate::global_safety::DebugMutexGuard;
use std::{
    borrow::Cow,
...
use crate::{
    ...
    common::{
        str2wcstring, unescape_string, unescape_string_in_place, valid_var_name, wcs2zstring,
        write_loop, CancelChecker, UnescapeStringStyle,
    },
    env::{
        EnvDyn, EnvDynFFI, EnvMode, EnvStack, EnvStackRef, EnvStackRefFFI, Environment,
        EnvironmentRef,
    },
    expand::{expand_one, ExpandFlags},
    fallback::fish_mkstemp_cloexec,
    fds::{wopen_cloexec, AutoCloseFd},
    ffi::{wcstring_list_ffi_t, Repin},
    flog::{FLOG, FLOGF},
    ...
    wutil::{
        file_id_for_fd, file_id_for_path, wgettext_fmt, wperror, wrealpath, wrename, wstat,
    ...
    fn drop(&mut self) {
        if let Ok(duration) = self.start.elapsed() {
            FLOGF!(
                ...
                "%s: %d.%06d ms",
                self.what,
                duration.as_millis() as u64, // todo!("later: remove cast")
                (duration.as_nanos() / 1000) as u64  // todo!("later: remove cast")
            )
        } else {
...
static s_histories: Mutex<BTreeMap<WString, Arc<History>>> = Mutex::new(BTreeMap::new());
...
impl HistoryImpl {
    ...
    fn timestamp_now(&self) -> SystemTime {
        ...
        if when == self.boundary_timestamp {
    ...
    fn rewrite_to_temporary_file(&self, existing_fd: Option<RawFd>, dst_fd: RawFd) -> bool {
            ...
            if let Some(local_file) = HistoryFileContents::create(existing_fd) {
                ...
                while let Some(offset) = local_file.offset_of_next_item(&mut cursor, None) {
                    ...
                    if old_item.timestamp() > self.boundary_timestamp {
                        if old_item.empty() || self.deleted_items.contains_key(old_item.str()) {
                            continue;
                        }
        ...
        // Write them out.
        let mut err = None;
        let mut buffer = String::with_capacity(HISTORY_OUTPUT_BUFFER_SIZE + 128);
        for item in items {
    ...
    fn save_internal_via_rewrite(&mut self) {
        ...
        for _i in 0..MAX_SAVE_TRIES {
            ...
            if !target_fd_before.is_valid()
                || !self.rewrite_to_temporary_file(Some(target_fd_before.fd()), tmp_fd)
            {
    ...
    fn save_internal_via_appending(&mut self) -> bool {
        ...
        if history_fd.is_valid() {
            ...
            while self.first_unwritten_new_item_index < self.new_items.len() {
                ...
                if item.should_write_to_disk() {
                    append_history_item_to_buffer(item, &mut buffer);
                    err = flush_to_fd(&mut buffer, history_fd.fd(), HISTORY_OUTPUT_BUFFER_SIZE);
                    if err.is_err() {
                        break;
                    }
                }
            ...
            if err.is_ok() {
                err = flush_to_fd(&mut buffer, history_fd.fd(), 0);
            }
    ...
    fn remove(&mut self, str_to_remove: WString) {
        ...
        let mut idx = self.new_items.len();
        while idx > 0 {
            idx -= 1;
    ...
    fn populate_from_bash<R: BufRead>(&mut self, contents: R) {
        ...
        for line in contents.lines() {
            ...
            let wide_line = str2wcstring(line.trim().as_bytes());
    ...
    /// This may be long!
    fn get_history(&mut self, result: &mut Vec<WString>) {
...
fn do_1_history_search(
    ...
    let mut searcher = HistorySearch::new(
        hist,
        search_string,
        search_type,
        if case_sensitive {
...
fn format_history_record(
    ...
    if let Some(time_format) = show_time_format {
        let time: DateTime<Local> = item.timestamp().into();
        result.push_str(&time.format(time_format).to_string());
    }
...
impl History {
    fn imp(&self) -> DebugMutexGuard<HistoryImpl> {
        DebugMutexGuard::new(self.0.lock().unwrap())
    }
    ...
    fn add(&self, item: HistoryItem, pending: bool) {
        self.imp().add(item, pending, true)
    }

    /// Returns history with the given name, creating it if necessary.
    pub fn with_name(name: &wstr) -> Arc<Self> {
        let mut histories = s_histories.lock().unwrap();

        if let Some(hist) = histories.get(name) {
            Arc::clone(hist)
        } else {
            let hist = Arc::new(Self(Mutex::new(HistoryImpl::new(name.to_owned()))));
    ...
    pub fn add_pending_with_file_detection(
        ...
        let item = HistoryItem::new(s.to_owned(), imp.timestamp_now(), identifier, persist_mode);
    ...
    pub fn search(
        ...
        if search_args.is_empty() {
            ...
            for search_string in search_args {
...
impl HistorySearch {
    ...
    pub fn new(
        ...
        if s.ignores_case() {
            s.canon_term = s.canon_term.to_lowercase();
        }
...
pub fn save_all() {
    for hist in s_histories.lock().unwrap().values() {
        hist.save();
    }
}
...
pub fn start_private_mode(vars: &EnvStack) {
    vars.set_one("fish_history"L, EnvMode::GLOBAL, ""L.to_owned());
    vars.set_one("fish_private_mode"L, EnvMode::GLOBAL, "1"L.to_owned());
}
...
static CHAOS_MODE: RelaxedAtomicBool = RelaxedAtomicBool::new(false);
